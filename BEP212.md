...
* Refunds increase block size variance. The theoretical maximum amount of actual gas consumed in a block is nearly twice the on-paper gas limit (as refunds add gas space for subsequent transactions in a block, though refunds are capped at 50% of a transaction's gas used). 

## Status

This BEP is a draft.  The code for this BEP has already been merged from upstream go-ethereum codebase into BSC node codebase.  The work entails enabling this EIP for BSC network configurations and perform hard fork.
...
2. Replace `SSTORE_CLEARS_SCHEDULE` (as defined in [EIP-2200](https://github.com/ethereum/EIPs/blob/9e393a79d9937f579acbdcb234a67869259d5a96/eip-2200.md)) with `SSTORE_RESET_GAS + ACCESS_LIST_STORAGE_KEY_COST` (4,800 gas as of [EIP-2929](https://github.com/ethereum/EIPs/blob/9e393a79d9937f579acbdcb234a67869259d5a96/eip-2929.md) + [EIP-2930](./eip-2930.md))
...
In [EIP-2200](https://github.com/ethereum/EIPs/blob/9e393a79d9937f579acbdcb234a67869259d5a96/eip-2200.md#specification), three cases for refunds were introduced:
...
A criticism of earlier refund removal EIPs ([EIP-3298](https://github.com/ethereum/EIPs/blob/9e393a79d9937f579acbdcb234a67869259d5a96/eip-3298.md) and [EIP-3403](https://github.com/ethereum/EIPs/blob/9e393a79d9937f579acbdcb234a67869259d5a96/eip-3403.md)) is that these EIPs fully remove the incentive to set a value to zero, encouraging users to not fully clear a storage slot if they expect even the smallest probability that they will want to use that storage slot again.
