...
- [BEP-336: Implement EIP-4844: Shard Blob Transactions](#bep-336-implement-eip-4844-shard-blob-transactions)
  ...
  - [6. Rationale](#6-rationale)
    ...
    - [6.2 Why Different Blob Size Per Block](#62-why-different-blob-size-per-block)
    - [6.3 Why Minimum/Maximum Blob Gas Price](#63-why-minimummaximum-blob-gas-price)
  - [7. Backwards Compatibility](#7-backwards-compatibility)
  - [8. Security Considerations](#8-security-considerations)
  - [9. Life of a Blob Transaction](#9-life-of-a-blob-transaction)
    - [9.1 Blob pool](#91-blob-pool)
    - [9.2 Verification / Filtering](#92-verification--filtering)
    - [9.3 Consensus process: Generation \& Storage of Sidecars](#93-consensus-process-generation--storage-of-sidecars)
    - [9.4 Propagation of Sidecars among peers](#94-propagation-of-sidecars-among-peers)
    - [9.5 Fetching of Sidecars when a node starts](#95-fetching-of-sidecars-when-a-node-starts)
  - [10. Changes Specifc to BSC](#10-changes-specifc-to-bsc)
    - [10.1 Verification](#101-verification)
    - [10.2 Sidecar Propagation / P2P network](#102-sidecar-propagation--p2p-network)
    - [10.3 Sidecar Fetching/Downloading](#103-sidecar-fetchingdownloading)
...
| `MAX_BLOB_GAS_PER_BLOCK` | `1048576` |
| `TARGET_BLOB_GAS_PER_BLOCK` | `524288` |
| `MIN_BLOB_GASPRICE` | `1` |
...
The actual `data_fee` as calculated via `calc_data_fee` is deducted from the sender balance before transaction execution and is not refunded in case of transaction failure. But unlike Ethereum, the `data_fee` will not be burnt on BSC, as BSC has different tokenomics design.
...
### 6.2 Why Different Blob Size Per Block

For Ethereum, the target blob size of each block is 3 and maximum is 6. As BSC has larger network capacity, the size will be increased slightly, which will be target 4 and maximum 8. 
Take opBNB’s traffic on 2024-02-27 for example, its average GasUsed per block is around 3.2M, it rollups ~117KB data to L1 every 15 seconds, equal to ~23.4KB for each block. For large traffic on opBNB, like 100M gas per block, it will be ~731KB(23.4*100/3.2), that is ~5.85 blobs(731KB/125KB). So 6 blobs per block will be merely enough to handle the maximum traffic of opBNB, if there are other L2s, then 6 blobs could be not enough, while 8 blobs per block could be enough to handle large traffic on opBNB. The capacity could be adjusted in the future, according to the overall network performance.

### 6.3 Why Minimum/Maximum Blob Gas Price
...
The 50Gwei MAX_BLOB_GASPRICE_BSC is to make sure the maximum blob cost will be similar to the cost of calldata.
...
This BEP increases the bandwidth requirements per block by a maximum of 1 MB.
This is ~11.4% larger than the theoretical maximum size of a block today (140M gas / 16 gas per calldata byte = ~8.345M bytes), and so it will not greatly increase worst-case bandwidth.
Basically, BSC has a good network bandwidth, it may not be a big issue to support the extra 1MB.


## 9. Life of a Blob Transaction
...
### 9.3 Consensus process: Generation & Storage of Sidecars
...
### 9.4 Propagation of Sidecars among peers
...
### 10.2 Sidecar Propagation / P2P network
...
### 10.3 Sidecar Fetching/Downloading
