...
Unlike Ethereum, block & blobs are propagated atomically, either of the following approach is ok:
- a.Blobs could be broadcasted together with block by a single message, in which case, the broadcast message will be rlp encoded as:
```
...
- b.Block and blobs can also be broadcasted separately to reduce the network traffic, e.g. broadcast the block first, but if the receiver does not have the corresponding sidecars, it needs to request the missing sidecars through another query message. This would also guarantee availability of the blobs.
