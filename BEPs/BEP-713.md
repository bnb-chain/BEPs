...
**No hardfork. No user-visible change. Prerequisite for the rest.**

**The problem.** BSC encodes *which algorithm* implicitly almost everywhere — by field position and length. A 65-byte signature in a transaction is ECDSA because nothing else could occupy that field; a 48-byte validator vote key is BLS12-381 for the same reason. Nothing on the wire names the algorithm, so a verifier cannot be *told* to use a different one; the field can only be replaced wholesale. EIP-4844's versioned hash, whose leading `0x01` byte declares which commitment scheme produced it, is the one place BSC already does this right, and it is the pattern worth generalising.

**Why it is a phase of its own** rather than a task inside each later phase: every phase needs the same mechanism, and if each solves it alone they will solve it differently. BEP-708 already carries a `kem-group` identifier with its own registry of valid values; a Phase 2 transaction type would invent its own signature-type byte; Phase 3 would invent a third convention. Allocating identifiers once avoids three incompatible schemes — and avoids the second format break that follows when any individual choice has to change. Given Phase 3's dependence on unresolved research, at least one of them will.

Deliverables:

- **Algorithm registry** — identifiers for signatures, KEMs, and commitments, plus the governance process for adding and deprecating entries.
- **Versioned envelopes** — self-describing `(algorithm, bytes)` encodings for keys, signatures, and commitments, so verification dispatches on a declared identifier instead of on field width. Applied to transaction signatures, account key-binding records, validator vote keys, and the discovery identity scheme.
- **Library and dependency policy** — which implementations are acceptable in consensus-critical code, constant-time and side-channel requirements, FIPS posture, and audit expectations.
- **Shared test vectors and a conformance harness** — reused by every later phase, so client implementations are checked against the specification rather than against each other.

None of this changes behaviour on its own, which is precisely why it is cheap now and expensive later: adding an identifier to a format that has not yet shipped costs nothing, while adding one to a format already in use costs a fork.
