...
func (p *Parlia) VoteTarget(chain consensus.ChainHeaderReader, curHead *types.Header) (*types.Header, error) {
	...
	// Case 1: If the current block number is beyond justified+depth, vote for the current head
	if latestJustified+depth < curNum {
		return curHead, nil
	}

	// Case 2: If the parent's justified number equals the current justified, no vote is required
	...
	// Case 3: If finalized+1 equals justified, vote for the current head
	...
	// Case 4: Check whether all intermediate blocks [latestJustified+1 .. curNum-1) can be skipped
	allSkipped := true
	...
	// Case 5: vote for the current head by default
	return curHead, nil
}
```
...
| ----------- | ---------------- | ---------------------------- | ---------------- | ---------------- | ---------------------------------------------------------------- |
| 3           | -                | skip                         | 0                | 0                | Case 2: parent justified equals latest justified → no vote       |
| 4           | -                | 0 → 4                        | 0                | 0                | Case 1: current block far ahead → vote current head              |
| 5           | -                | 0 → 5                        | 0                | 0                | Case 1: current block far ahead → vote current head              |
| 6           | 0 → 4            | 4 → 6                        | 4                | 0                | Case 5: default → vote current head                              |
| 7           | -                | skip                         | 4                | 0                | Case 2: parent justified equals latest justified → no vote       |
| 8           | 4 → 6            | 6 → 7                        | 6                | 0                | Case 4: all intermediate blocks skipped → vote latestJustified+1 |
| 9           | -                | skip                         | 6                | 0                | Case 2: parent justified equals latest justified → no vote       |
| 10          | 6 → 7            | 7 → 10                       | 7                | 6                | Case 3: finalized + 1 == latest justified → vote current head    |
| 11          | -                | skip                         | 7                | 6                | Case 2: parent justified equals latest justified → no vote       |
| 12          | 7 → 10           | 10 → 11                      | 10               | 6                | Case 4: all intermediate blocks skipped → vote latestJustified+1 |
| 13          | -                | skip                         | 10               | 6                | Case 2: parent justified equals latest justified → no vote       |
