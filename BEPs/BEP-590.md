...
* As the block interval decreases or the GasLimit increases, **D** grows. The following analyzes the justified and finalized status of blocks:

From the example in Section 4.1, votes alternate between **justifying the latest head** and **advancing the finalized height**.
...
Additionally, to ensure continuous growth of the finalized block number, it is required that:
```
