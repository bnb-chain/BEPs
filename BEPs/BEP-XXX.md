...
# BEP-XXX: Enhanced Fast Finality via In-Memory Voting Pool

- [BEP-XXX: Enhanced Fast Finality via In-Memory Voting Pool](#bep-xxx-enhanced-fast-finality-via-in-memory-voting-pool)
	- [1. Summary](#1-summary)
...
This BEP proposes an optimization to the Fast Finality mechanism introduced in [BEP-126](./BEP126.md), enabling nodes to collect attestations directly from in-memory vote pools rather than waiting for them to be packaged into block headers, thereby achieving immediate finality determination.
...
Block headers continue to include attestations for backward compatibility.

## 3. Motivation

Under BEP-126, finality determination is unnecessarily coupled to block production. Validators vote within milliseconds, but finality cannot be determined until attestations are packaged into block headers. Once ⅔+ validators have voted, their votes already provide sufficient cryptographic proof—waiting for block inclusion introduces artificial delay.
