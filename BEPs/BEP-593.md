...
Additional incremental snapshots generation flag:
...
- `incr.state-buffer` is used to set the incr state memory buffer to aggregate MPT trie nodes. The larger the setting, the smaller the incr snapshot size. The default value is 6GB.
