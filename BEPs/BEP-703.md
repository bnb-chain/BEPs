...
- [BEP-703: Payment Lane on BNB Smart Chain](#bep-703-payment-lane-on-bnb-smart-chain)
  ...
  - [3. Specification](#3-specification)
    ...
    - [3.8 System Contract](#38-system-contract)
      - [3.8.1 Installation and Storage](#381-installation-and-storage)
      - [3.8.2 Reading](#382-reading)
      - [3.8.3 Updating](#383-updating)
      - [3.8.4 Enforcing the Invariants](#384-enforcing-the-invariants)
...
- **Ratios** are integers against `RATIO_DENOM = 10000`: a stored `N` means `N / 10000`. What values a ratio may hold is [§3.6](#36-parameters-invariants-and-governance)'s business, not the arithmetic's.
...
The governable parameters of [§3.6](#36-parameters-invariants-and-governance) and the payment contract list of [§3.7](#37-payment-contract-list) live in one system contract, `PaymentLane`, at `0x0000000000000000000000000000000000002007`.

#### 3.8.1 Installation and Storage

The fork installs the code at that address while block `activation` executes ([§3.4.5](#345-activation)), through the same system-contract upgrade path every earlier BSC fork has used. There is no initializer and no genesis storage: an unwritten slot reads as zero, zero is not a settable value for any of the eight parameters, and a parameter reading zero takes the value [§3.6](#36-parameters-invariants-and-governance) lists for it. All-zero storage therefore already *is* the configuration of §3.6, so installing the code is the whole of activation.
...
Changes arrive through a single entry point, `updateParam(string key, bytes value)`, callable only by the governance hub at `0x0000000000000000000000000000000000001007` and so reached by the ordinary BSC governance path: a proposal on the governor at `0x0000000000000000000000000000000000002004`, executed by the timelock at `0x0000000000000000000000000000000000002006` once it passes. Eight keys name the parameters of §3.6 — each its §3.6 name in lower camel case, `paymentLaneMinRatio` and so on — and carry a 32-byte value; two more, `addPaymentContract` and `removePaymentContract`, carry a 20-byte address. Adding an address already listed, or removing one that is not, reverts rather than passing silently, so that every accepted call is a real change.

An accepted call lands in the post-state of the block that carries it. Every rule here reads the *parent's* post-state — §3.2 for list membership, §3.4 for the parameters — so a change is invisible to its own block and governs from the next one onward. That is the boundary [§3.7](#37-payment-contract-list) states for listings, and it holds identically for parameters.

#### 3.8.4 Enforcing the Invariants

[§3.6](#36-parameters-invariants-and-governance) says a proposal violating an invariant is invalid and never activates. As contract behaviour: **it reverts at execution.** A numeric update is applied to an in-memory copy of the whole eight-value tuple, the tuple is validated as a whole, and storage is written only if it passes — a failure reverts with no slot touched, so no violating tuple is ever stored, whether to be ignored or checked later. The whole tuple is revalidated on every key because the invariants couple all eight: moving one parameter can break an invariant it does not itself appear in. What an observer sees is a failed governance transaction and unchanged on-chain parameters.

Validation has two parts. The first is the six invariants of §3.6. The second is a set of fixed range guards, which bound each parameter on its own where §3.6 bounds them only against each other:

| Guard | Bound |
|---|---|
| every one of the eight | non-zero — this is how "never written" is encoded |
...
- **Parameter safety.** The invariants in [§3.6](#36-parameters-invariants-and-governance) are validated before any parameter change activates, so no governance action can invert the two thresholds, invert the two steps, take a step wide enough to cross the hysteresis band, or set a maximum the quota could never reach. Those six bind the parameters only against each other; the range guards of [§3.8.4](#384-enforcing-the-invariants) bound each absolutely, which is what keeps the reservation from being voted to a share of the block no valid block could carry. What neither set guarantees is a wide effective range (invariant (3)) — but a narrow one costs only the dynamic behaviour; no rule is violated.
