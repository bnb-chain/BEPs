...
A parameter tuple violating [§3.6.2](#362-invariants-and-range-guards) is the same kind of corruption, and the node MUST check for it: it MUST validate every tuple it reads against §3.6.2's range guards and invariants, in that order, and MUST reject the block when the tuple fails. The contract of [§3.6.4](#364-the-system-contract) cannot store such a tuple, so reading one means the code at that address is not the contract this specification defines. Every client MUST apply this check — one that only some clients apply is a split rather than a safeguard — and every client MUST use §3.6.2's constants unchanged, since a client holding a stale bound rejects blocks its peers accept. Note the consequence: a chain that reaches this state produces no further blocks until the code at §3.6.4's address is replaced, and that halt does not clear through governance, because the transaction that would clear it needs a block.
