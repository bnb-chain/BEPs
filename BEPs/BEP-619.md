...
| BSCValidatorSet.TurnLength           | contract | 4                      | 8                | 16                | 8                   |
...
* **TurnLength**: TurnLength is reduced for two reasons. First, when certain validator nodes become isolated from the rest of the network, the resulting reorg depth can scale proportionally with TurnLength. Second, reducing consecutive block production from 16 to 8 decreases local block-building throughput by less than 10%. If builders exist in the ecosystem, this impact is expected to be negligible.
...
* **If fast finality fails** (natural finality with TurnLength=8, ValidatorSize=21):
  ...
  * > 1/2 validator confirmations: 88 blocks (11×8) ≈ 39.6s
  * > 2/3 validator confirmations: 120 blocks (15×8) ≈ 54s
