...
**For blocks where `N % 10 != 0` (non-eligible blocks):**
- `header.ExcessBlobGas` MUST equal `parent.ExcessBlobGas`
- `header.BlobGasUsed` MUST equal `parent.BlobGasUsed`

**For blocks where `N % 10 == 0` (eligible blocks):**
- Normal EIP-4844 blob fee calculation applies

This ensures the blob fee market mechanism remains continuous across non-eligible blocks, allowing proper fee calculation when the next eligible block arrives.
...
This is a consensus-breaking change requiring a hard fork. L2 sequencers must adjust their data availability posting schedules to target eligible blocks (maximum additional delay: 4.05 seconds). Blob-related MEV opportunities will concentrate in eligible blocks.
