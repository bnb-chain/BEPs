<pre>
  ...
  Type: Standards
  Created: 2026-01-21
  Description: Restrict blob transactions to blocks where block number modulo N equals zero (N initially set to 5)
</pre>
...
By restricting blob transactions to every Nth block (blocks where `blockNumber % N == 0`, with N initially set to 5), this BEP concentrates demand to restore proper fee market dynamics while maintaining adequate capacity for current usage patterns. With 0.45s block times, the maximum inclusion delay of 2.25 seconds (for N=5) has minimal impact on L2 operations.

## 3. Specification

### 3.1 Blob Transaction Inclusion Rules

Define a parameter N (initially set to 5).
...
The modulo N restriction (1 in every N blocks, with N initially set to 5) aligns with current demand patterns:
...
* **Minimal User Impact**: With 0.45s block time, maximum inclusion delay is 2.25 seconds (for N=5), negligible for L2 sequencers posting data every 30-60 seconds.
* **Fee Continuity**: Non-eligible blocks inherit blob fee state from their parent, preserving market state across the (N-1) block gap and enabling proper price discovery at the next eligible block.
...
Transaction submitters must ensure blob transactions target eligible blocks (blocks where `blockNumber % N == 0`). With the initial N=5, eligible blocks are 0, 5, 10, 15, etc. Blob-related MEV opportunities will concentrate in these eligible blocks.
