...
This proposal shortens affected turns and removes failed validators from the rotation earlier. It rebalances rather than preserves the recovery window: the time before losing accrued rewards stays roughly unchanged, while the time before being jailed shortens. Section 4.2 quantifies both.
...
Elapsed time differs from the affected-interval sums above because the other 20 validators keep producing between the failed validator's turns. Each missed block advances the chain by about `21 * 0.45 + 2 = 11.45 s` today and `21 * 0.45 + 1 = 10.45 s` under the proposal. Under the current configuration a failing validator therefore reaches the misdemeanor threshold after about 63.5 minutes (`333 * 11.45 s`), enters maintenance through that misdemeanor, and reaches felony after a further 5.25 hours (`1000 * 21 * 2 * 0.45 s`). If maintenance is unavailable, ordinary slashing reaches felony after about 3.2 hours (`1000 * 11.45 s`). Under the proposal, the 95-count entry allowance corresponds to about 16.5 minutes (`95 * 10.45 s`).

| Interval from failure | Current | Proposed |
| --- | --- | --- |
| To misdemeanor (loss of accrued income) | about 63.5 minutes | about 66 minutes |
| To felony through maintenance | about 6.3 hours | about 4.3 hours |
| To felony without maintenance | about 3.2 hours | about 1.7 hours (`600 * 10.45 s`) |

The proposal keeps the window before income loss roughly unchanged and shortens the window before jailing by about a third. This trade-off is intended: the misdemeanor now falls inside maintenance, after the network has already been protected, while the felony window stays at several hours.
